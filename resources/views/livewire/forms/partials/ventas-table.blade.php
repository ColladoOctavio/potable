    ...
    @if($ventas->count())
        <div class="table-responsive">
            <table class="table table-hover">
                <thead><tr><th>Fecha</th><th>Empresa</th><th>Cliente</th><th>Descripcion</th><th>Kilos</th><th>Total</th><th>Estado</th><th class="text-end">Acciones</th></tr></thead>
                <tbody>
                @foreach($ventas as $venta)
                    <tr>
                        ...
                        <td>{{ number_format($venta->kilos, 0, ',', '.') }}</td>
                        <td>{{ $money($venta->importe_total) }}</td>
                        <td><span class="badge text-bg-info status-badge">{{ $venta->estado_cobro }}</span></td>
